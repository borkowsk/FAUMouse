...
int ILE_SEC=1; //Ile sekund czeka na OK

void ViewFile(HWND myWin,const char* filename,const char* directory=NULL)
{
	char starterf[1024];
	if(directory==NULL)
    {
        GetModuleFileName(NULL,starterf,1023);
	    directory=starterf;
        char* pom=strrchr(starterf,'\\');
    ...
    ShellExecute(myWin,"open",filename,NULL,directory,SW_SHOWNORMAL);
}		
...
void CheckLicenceOK()
{
    //Szukamy klucza swiadczacego o OK na licencje
    HKEY	KeyHandle=0;
    ...
    if(Result!=ERROR_SUCCESS)//Jesli nie bylo klucza
    {

        ViewFile(0,"licence.pdf");
    }
    
    if(KeyHandle!=NULL)				//Jesli byl
        RegCloseKey(KeyHandle);	//To zwalniamy
}

void MarkLicenceOK()
{
    //Ustrawianie klucza swiadczacego o OK na licencje
    HKEY	KeyHandle=0;
    long	Result=RegCreateKey(
        HKEY_CURRENT_USER,			
        SubKeyName,  
        &KeyHandle   // address of buffer for opened handle
        );
    ...
    if(Result!=ERROR_SUCCESS)//Jesli nie mozna bylo klucza utworzyc
    {
        MessageBox(0,"Can't open the registry key","Modification of the registry",MB_OK|MB_ICONSTOP);
    }
    else
    {
    RegSetValue(
        KeyHandle,         // handle to key to set value for
        "LicenceOK",  // address of subkey name
        REG_SZ,      // type of value
        "Yes",    // address of value data
        3       // size of value data
        );    
    }
}
